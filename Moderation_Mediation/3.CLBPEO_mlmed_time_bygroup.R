...
### 2️⃣ Change 2: Add Group×Time interaction on mediator (SmGroupTime = Sm × Group × time)
...
# Keep rows with Pain and Group
data <- subset(data, !is.na(Pain) & !is.na(Group))

# Group: 1 = Video, 0 = Zoom (instead of Tx: 1 = PE, 0 = WL)
data$Group_bin <- ifelse(data$Group == "Videos", 1, 0)
...
sink("model_mlmed_time_bygroup.txt")
...
for (mediator in mediators) {
  ...
  tmp <- data.frame(
    Group_bin = temp_data$Group_bin,  # Changed from Tx_bin to Group_bin
    Y         = temp_data$Pain,
  ...
  # Starting point: Classic Group & mediator effects (unchanged)
  tmp$SmGroup <- tmp$Sm * tmp$Group_bin        # baseline group gap in mediator (a_base)
  tmp$SyGroup <- tmp$Sy * tmp$Group_bin        # c' path (unchanged)
  ...
  # 2️⃣ CHANGE 2: Add Group×Time interaction on mediator
  tmp$SmGroupTime <- tmp$Sm * tmp$Group_bin * tmp$time  # Group×time on mediator = a_change (difference-in-differences)

  # Starting point: Fixed & random formulas (modified)
  # Original: "Z ~ 0 + Sm + Sy + SmTx + SyTx + SyM"
  # Modified: Added SyMw, SyMb, Smt, SmGroupTime
  fixed.formula <- "Z ~ 0 + Sm + Sy + SmGroup + SyGroup + SyMw + SyMb + Smt + SmGroupTime"
  ...
  # Original: a_effect <- extract.model[["SmGroup"]]  # baseline group difference
  # Modified: Now we have two a-paths:
  a_base   <- extract.model[["SmGroup"]]      # baseline group difference (a_base)
  a_change <- extract.model[["SmGroupTime"]]   # difference-in-differences (a_change)
  ...
  cprime_effect <- extract.model[["SyGroup"]]  # STARTING POINT: Unchanged
  ...
  cat("a_change (Group×time difference-in-differences):", a_change, "\n")
  cat("b-path (within mediator -> pain):", b_effect, "\n")
  cat("c'-path (Group -> pain):", cprime_effect, "\n")
  ...
  bootout <- function(dat, indices) {
    ...
    tmpb <- data.frame(
      Group_bin = ifelse(rdat$Group == "Videos", 1, 0),  # Changed from Tx to Group
      Y         = rdat$Pain,
      M         = rdat[[mediator]],
    ...
    tmpb$SmGroup     <- tmpb$Sm * tmpb$Group_bin      # baseline group gap
    tmpb$SyGroup     <- tmpb$Sy * tmpb$Group_bin      # c' path
    ...
    tmpb$SmGroupTime <- tmpb$Sm * tmpb$Group_bin * tmpb$time  # 2️⃣ CHANGE 2: Group×time interaction
    ...
    # Original: a <- em[["SmGroup"]], b <- em[["SyM"]]
    # Modified: Now returns a_change and within-person b-path
    a  <- em[["SmGroupTime"]]  # 2️⃣ CHANGE 2: a_change (Group×time interaction)
    b  <- em[["SyMw"]]         # 4️⃣ CHANGE 4: within-person b-path
    cp <- em[["SyGroup"]]      # STARTING POINT: c' path (unchanged)
  ...
  # Original: ci_a for SmGroup, ci_b for SyM
  # Modified: ci_a for SmGroupTime (a_change), ci_b for SyMw (within-person b-path)
  ci_a  <- boot.ci(bootmlm, index = 1, type = "perc")  # 2️⃣ CHANGE 2: a_change (Group×time)
  ...
  p_a  <- boot_p(bootmlm, 1)  # 2️⃣ CHANGE 2: p-value for a_change (Group×time)
  ...
  cat("a_change (Group×time) p-value:", p_a, "\n")      # 2️⃣ CHANGE 2: a_change p-value
...
for (mediator in mediators) {
  cat("MEDIATOR:", mediator, "\n")
  cat("a_base (SmGroup):", results[[mediator]]$extracted_effects[["SmGroup"]], "\n")           # baseline group difference
  cat("a_change (SmGroupTime):", results[[mediator]]$extracted_effects[["SmGroupTime"]], "\n") # difference-in-differences
  cat("b (SyMw):", results[[mediator]]$extracted_effects[["SyMw"]], "\n")                 # within-person b-path
  cat("c' (SyGroup):", results[[mediator]]$extracted_effects[["SyGroup"]], "\n")               # c' path
  cat("indirect_base (a_base × b):",
      results[[mediator]]$extracted_effects[["SmGroup"]] * results[[mediator]]$extracted_effects[["SyMw"]], "\n")
  cat("indirect_change (a_change × b):",
      results[[mediator]]$extracted_effects[["SmGroupTime"]] * results[[mediator]]$extracted_effects[["SyMw"]], "\n\n")
}

sink()
cat("Analysis complete! Results saved to model_mlmed_time_bygroup.txt\n")
