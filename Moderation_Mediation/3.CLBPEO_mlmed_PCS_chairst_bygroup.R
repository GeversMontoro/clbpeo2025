####### Multilevel mediation for PCS and Chairst30 with Group (Videos vs Zoom) #########
...
# Remove rows with missing values only for essential columns (Pain and Group)
# This creates a clean base dataset for each mediator analysis
data <- subset(data, !is.na(Pain) & !is.na(Group))

# Set up binary predictor variable for Group (1 for Videos, 0 for Zoom)
...
sink("model_mlmed_by_group.txt")
cat("=== MULTILEVEL MEDIATION ANALYSIS BY GROUP: Videos vs Zoom ===\n")
...
for (mediator in mediators) {
  ...
  tmp$SmGroup <- tmp$Sm * tmp$Group_bin
  tmp$SyGroup <- tmp$Sy * tmp$Group_bin
  tmp$SyM <- tmp$Sy * tmp$Md
  
  # Define the fixed and random formulas
  fixed.formula <- "Z ~ 0 + Sm + Sy + SmGroup + SyGroup + SyM"
  ...
  # Extract initial model effects for indirect paths if desired
  extract.model <- extract.modmed.mlm(list(model = mod_med_tmp, conv = TRUE))
  initial_indirect <- extract.model[["SmGroup"]] * extract.model[["SyM"]]  # Calculate initial indirect effect for Group_bin
  
  # Print extracted effects
  cat("### EXTRACTED EFFECTS for Mediator:", mediator, "###\n")
  cat("a-path (Group -> Mediator):", extract.model[["SmGroup"]], "\n")
  cat("b-path (Mediator -> Pain):", extract.model[["SyM"]], "\n")
  cat("c'-path (Direct effect):", extract.model[["SyGroup"]], "\n")
  ...
  bootout = function(data, indices) {
    ...
    tmp <- data.frame(
      Group_bin = ifelse(rdat$Group == "Videos", 1, 0),
      Y = rdat$Pain,
    ...
    # Return coefficients
    a_path <- extract.model[["SmGroup"]]
    b_path <- extract.model[["SyM"]]
    c_prime <- extract.model[["SyGroup"]]
...
for (mediator in mediators) {
  ...
  cat("a-path coefficient:", results[[mediator]]$extracted_effects[["SmGroup"]], "\n")
  cat("b-path coefficient:", results[[mediator]]$extracted_effects[["SyM"]], "\n")
  cat("c'-path coefficient:", results[[mediator]]$extracted_effects[["SyGroup"]], "\n")
  cat("ab-path (indirect) coefficient:", results[[mediator]]$extracted_effects[["SmGroup"]] * results[[mediator]]$extracted_effects[["SyM"]], "\n")
...
cat("Group analysis complete! Results saved to model_mlmed_by_group.txt\n")
