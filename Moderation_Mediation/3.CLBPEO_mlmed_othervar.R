####### Multilevel mediation for Other Mediators (excluding PCS and Chairst30) with Tx (PE vs WL) #########
### Includes EDD (with log transformation) and all other mediators ###
...
# List of mediators to analyze (excluding PCS, Chairst30, and EDD - including ISI and TakaiP)
mediators <- c("ISI", "TakaiP", "SLD", "GeneralDisability", "Health_status", "BMI", "Other_pain", 
               "TSK", "BDI", "SAI", "TAI", "Balance", "TUG", "WT4S", 
               "WT4F", "RAPA", "MeP_Balance", "MeP_chairst", "MeP_TUG", "MeP_Takai", 
               "MeP_WT4", "MEP")

# Open a file connection for comprehensive output
sink("model_mlmed_other_mediators.txt")
cat("=== MULTILEVEL MEDIATION ANALYSIS: Other Mediators (excluding PCS and Chairst30) ===\n")
cat("Seed set to: 1234 for reproducibility\n")
cat("Includes EDD (with log transformation) and all other mediators\n")
...
for (mediator in mediators) {
  ...
  # Standard handling for all mediators
  ...
  bootout = function(data, indices) {
    ...
    # Standard handling for all mediators in bootstrap
    tmp <- data.frame(
  ...
  # Perform bootstrapping (use 5000 iterations for all mediators)
  ...
  bootmlm <- boot(data = temp_data, statistic = bootout, R = bootstrap_iterations)
  print(bootmlm)
  cat("\n")
...
cat("Other mediators analysis complete! Results saved to model_mlmed_other_mediators.txt\n")
